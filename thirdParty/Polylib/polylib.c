...
LIBRARY ROUTINES FOR ORTHOGONAL POLYNOMIAL CALCULUS AND INTERPOLATION
  ...
  jacobiP     Returns normalized Jacobi poly.
...
/*
  \brief Normalized Jacobi polynomial
  \author li12242
*/
void jacobiP(int np, double *z, double *poly_in, int n, 
  double alpha, double beta){
  double r;
  int i;
  jacobfd(np, z, poly_in, NULL, n, alpha, beta);
  r = (2*n+alpha+beta+1)/pow(2,alpha+beta+1)*gammaF(n+alpha+beta+1)*factorial(n)
      /gammaF(n+alpha+1)/gammaF(n+beta+1);
  for(i=0; i<np; i++){
    poly_in[i] = poly_in[i]*sqrt(r);
  }
  return;
}

void GradjacobiP(int np, double *z, double *dp, int n, double alpha, double beta){
  int i;
  double r;
  if(n<1){
    for(i = 0; i<np; i++){
